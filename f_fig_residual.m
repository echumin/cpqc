...
function f_fig_residual(path2EPI,funcvolpaths,qcpath,scanID,linkdir)
...
for ii=1:height(funcvolpaths)
    ...
    func_vol = niftiread(fullfile(funcvolpaths(ii).folder,funcvolpaths(ii).name));
    [~,~,~,T] = size(func_vol);
    rt=extractBetween(funcvolpaths(ii).name,'epi_','_DCT');
    ...
    sgtitle([scanID{1} ' ' scanID{2} ' ' funcvolpaths(ii).name],'Interpreter','none','Color','white')

    fileout = fullfile(qcpath,[scanID{1} '_' scanID{2} '_8-' rt{1} '_tissueType_resid']);
